...
export default function CountryView() {
  ...
  let getChecked = (e) => {
    if (e.target.checked) {
      ...
      let filterData = ids.filter((value) => value != e.target.value)
      setIds(filterData)
    }
  }
  ...
  let allChecked = (e) => {
    if (e.target.checked) {
      let finalData = countryData.map((val) => val._id)
      setIds(finalData)
    } else {
      setIds([])
    }
  ...
  useEffect(() => {
    if (ids.length == countryData.length && ids.length > 0) {
      setAllChecked(true)
    } else {
      setAllChecked(false)
    }
          ...
          <div className={`rounded-xl shadow-xl border-t-1 border-[#eee] p-5 bg-blend-hue`}>
            <div className="flex items-center justify-between mb-3">
              <div className='flex gap-3'>
                <h2 className="text-lg font-bold">View Country</h2>
              ...
              <span className="flex gap-1">
                ...
                <button className='bg-red-600 rounded p-2 text-white cursor-pointer' onClick={countryDelete}>DELETE</button>
              </span>
            </div>
            <div>
              <table className="table-auto w-full border bg-[#1f2937] text-white pb-5">
                <thead>
                  <tr className='bg-blue-950'>
                    <th className="border px-4 py-2 text-left"> <input checked={allCheck} onChange={allChecked} type="checkbox" /> Country Form</th>
                ...
                <tbody>
                  {
                    countryData.length > 0 ? (
                      countryData.map((data, index) => {
                        return (
                          <tr className='hover:bg-[#2c3643] cursor-pointer' key={index}>
                            <td className="border px-4 py-2"><input checked={ids.includes(data._id)} value={data._id} onChange={getChecked} type="checkbox" /> {data.countryName}</td>
                            <td className="border px-4 py-2">{data.countryOrder}</td>
                            {
                              data.countryStatus ?
                            ...
                            <td className="border px-4 py-2"><Link to={`/country/edit/${data._id}`}><button className='bg-blue-600 rounded p-3 cursor-pointer'><FaPenAlt /></button></Link></td>
                      ...
                      (
                        <tr>
                          <th colSpan={4} className='text-center'>Data Not Found..!</th>
                        </tr>
                      )
                  }
